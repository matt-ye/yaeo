...
> ✅ **L1／L2 那批欄位在 AI 搜尋管線裡有實測支持（2026-08-17 新增）。**
> [arXiv 2602.12187](https://arxiv.org/abs/2602.12187)（SAGEO Arena）把
> **title／meta description／H1–H6／schema-JSON-LD** 當成分開索引的欄位
> （照真實搜尋引擎的做法），比較「只優化這些結構欄位」與「只優化正文」：
> 前者 **+22% Hit Rate、平均檢索排名 +2.72**；後者一致退化。
>
> 這件事值得標出來，因為[綜述](https://arxiv.org/abs/2607.14035)說 GEO 的證據
> 「未證實自然可發現性」——也就是碰不到「會不會被撈到」。**這篇碰到了，
> 而且量的正是 L1／L2 已經在檢查的東西。** 原本這批規則只有 Google 官方文件
> 這個「應該這樣做」的依據，現在多了一個管線內的實測。
>
> ⚠ **但不改任何嚴重度。** 它比較的是兩種**優化策略**的效果，
> 不是「缺這個欄位會怎樣」。把策略比較的結論搬去當缺漏的罰則，就是過度推論。
...
> [arXiv 2604.19113](https://arxiv.org/abs/2604.19113)（FeatGEO）把網頁抽象成 13 維特徵
> 並逐項消融，結論是**貢獻 substantial 的是 Statistics 與 Cite Sources**、
> minimal 的是 Technical Terms，整體由 content 層特徵主導。
> 也就是說這裡數的三項，其中兩項正好是 ablation 裡最強的兩項。
>
> **嚴重度仍然不變。** FeatGEO 一樣沒有給閾值，`info` ＋ 不給數字的做法照舊。
> 有 ablation 支持「這幾項有影響」，不等於有依據說「幾個才夠」。

#### 版面結構：加進訊息，但不變成判準

三篇研究指向同一件事——長篇散文缺清單與表格會降低被引用的機率：
FeatGEO 的 `list_density`、[SAGEO Arena](https://arxiv.org/abs/2602.12187) 的修復工具
「加標題階層與清單」、以及[引用失敗分類](https://arxiv.org/abs/2603.09296)裡的
「版面無結構」（屬於內容品質那 27.1%）。

**處理方式是把數字加進 `L3-GEO-SIGNALS-*` 既有的訊息，不另立規則。**
報告現在會多一句「版面 清單/表格 N 處、次級標題 N 個，未列入判定」。

為什麼不立新規則：三篇**沒有一篇給閾值**（多長算長篇、幾個清單算夠），
而沒有閾值就只能報 `info` 且不給數字——那樣它的訊號會和 `L2-THIN-CONTENT`
高度重疊，同一批長而扁的頁面被念兩次，只是多一條噪音。
完整理由記在 `watch/investigated.json` 的 `list-density-rule`。

> ⚠ **已知代價，寫下來免得以後當成 bug：** 三項訊號都齊備的頁面，
> `NONE` 與 `THIN` 都不觸發，於是版面數字也不會出現。
> 附註掛在既有訊號上，就只能跟著它出現。這是「不另立規則」換來的。
...
### 「結構到底有沒有用」的衝突：其實是階段混淆（2026-08-17，2026-08-17 二訂）
...
補讀另外兩篇之後，矛盾大致消失——**它們量的是管線的不同階段，也是不同層次的「結構」**：

| 改什麼 | 在哪個階段 | 效果 | 出處 |
|---|---|---|---|
| 表層詞彙／格式改寫 | 引用 | 極小，甚至有害（AutoGEO 檢索 **−36%**、Technical Terms **−14%**） | 2605.25517、2602.12187 |
| **結構化 metadata 欄位**（title／description／H1–H6／JSON-LD） | **檢索** | **+22% Hit Rate、排名 +2.72** | 2602.12187 |
| **文件層級的內容屬性**（統計、引用來源、獨特資訊） | 引用 | 顯著；ablation 中 Statistics 與 Cite Sources 貢獻最大 | 2604.19113、2311.09735 |

**處置不變：不新增規則，也不改嚴重度。** 但理由變了——原本是「兩篇打架所以不動」，
現在是「三個層次各自成立，而它們對應的檢查 L1／L2／L3 都已經有了」。

值得注意的是這些結論落在哪一邊：**它們支持現行做法。** 既然表層改寫無效甚至有害，
`L3-GEO-*` 那三項就不該升級成硬規則、也不該給閾值。這次查證的產物是
「原本的克制是對的」，不是一條新規則——這種結果一樣要寫下來，
否則下一個人會以為這個主題沒人查過。

> **另一個還沒解決的衝突。** [2511.20867](https://arxiv.org/abs/2511.20867) E-GEO 主張
> 存在「普遍有效的 GEO 策略」（domain-agnostic pattern），而
> [2607.14035](https://arxiv.org/abs/2607.14035) 綜述說「通用啟發法轉移性差」。
> [2604.19113](https://arxiv.org/abs/2604.19113) FeatGEO 報告效果因領域而異，站綜述那邊。
> 目前 **2 比 1**，不改任何立場，只記著。
...
5. **最後附〈修完怎麼確認〉**——腳本會自己印，別漏掉（見下節）

### 修完怎麼確認（驗證層級）

**這份報告原本只回答「哪裡不對」，不回答「修了怎麼知道有效」。**
使用者實際會做的事是重跑腳本、看訊息消失，然後把那當成有效果。
那是兩件事，而把它們混為一談是這支腳本原本默許的誤解：

| | 意思 | 適用範圍 |
|---|---|---|
| **訊息消失** | 建置產物裡的事實改掉了 | 所有規則，確定性 |
| **有沒有效果** | 搜尋引擎／AI 的行為變了 | **要分規則** |

第二欄分兩級，由 `seo-check.mjs` 的 `VERIFY_TIERS` 定義，報告尾端會依
本次實際觸發的層級印出：

| 層級 | 涵蓋 | 怎麼看效果 |
|---|---|---|
| `rerun-then-console` | L1／L2／SITE／`L3-AI-SNIPPET-*` | Search Console 的收錄與爬取資料（有延遲，數天到數週） |
| `no-single-shot` | `L3-GEO-*` | **無法用單次量測驗證** |

**為什麼 L3-GEO 那格寫「驗不了」而不是給一個驗法。**
[arXiv 2604.07585](https://arxiv.org/abs/2604.07585) 在四個引擎上跑 45–46 天，
來源集合的日間 Jaccard 只有 0.34–0.42（**每天約 65% 的來源會換掉**），
同一天內同時重跑也只有 0.32–0.43；作者建議至少 7 runs/prompt/日、觀測窗 2–4 週。
**在那種雜訊水準下，給一個「改完再查一次」的驗法就是在教人做錯的量測。**
誠實的答案是「你驗不了」，而那比一個假的驗法有用——這也和
`L3-GEO-*` 本來就只報 `info`、不給目標數字是同一個理由。

> ⚠ **這一節由測試強制。** `rule-index.test.mjs` 斷言每條規則都歸到了某個
> 驗證層級，新增規則卻沒歸類就會失敗並指名；同時斷言本節存在。
>
> 為什麼要強制：[claude-seo](https://github.com/AgriciDaniel/claude-seo) 的 README
> 宣稱「每條建議都帶可證偽檢查」，但實測 grep 它的 `schema`／`tests`／`scripts`
> **零命中**——那個要求只寫在給模型看的提示詞裡。
> **要做同一件事，就得先解決它沒解決的那一半：讓它成為斷言，不是期望。**
...
> **這和上面那個「結構欄位 +22%」不衝突，因為量的不是同一件事。**
> 62.2% 說的是「**引用失敗**的原因分佈」，+22% 說的是「優化結構欄位對
> **被檢索到**的效果」。一份頁面可以順利被撈進候選集（結構欄位幫得上），
> 然後在生成答案時因為答錯了問題而沒被引用（結構欄位幫不上）。
> 兩個數字擺在一起看很容易讀成矛盾——**先確認它們講的是管線的哪一段。**
...
**① 站外訊號，而且它可能比站內更重要。**
[arXiv 2509.08919](https://arxiv.org/abs/2509.08919)（1,000 個消費類排序查詢、
US 與 CA、四個引擎）量到 AI 搜尋對第三方來源的壓倒性偏好：

| 領域 | AI 搜尋的 earned media 佔比 | Google 對照 |
|---|---|---|
| 汽車（CA） | **69.1%**（brand 30.9%、social 0%） | earned 40.6%、brand 36.6% |
| 消費電子（US） | **92.1%** | brand 32.9%、social 15.4% |
| 軟體（CA） | **74.2%**（social 0%） | brand 53.8% |

同一效應另有兩篇獨立指向：[2601.00912](https://arxiv.org/abs/2601.00912)
發現有預測力的是 referring domains（+0.319）與社群聲量（+0.395），
而 GEO 分數與被發現率零相關；[2603.12282](https://arxiv.org/abs/2603.12282)
在 UK iGaming 上複製了同樣的偏好。

**意思很直接：這份報告全過，仍然可能因為沒有第三方報導而不被引用。**
本檢核器只讀單一站台的建置產物，站外聲量在它的視野之外。
這比上面那個 62.2% 更難堪，但更該講——一份檢核報告最容易造成的誤解，
就是讓人以為「全綠＝做完了」。

**那該怎麼辦？** 這一層的工作不是改頁面，是**讓別人有東西可以引用你**：
把可被引用的東西（原始數據、可複製的方法、明確的立場）寫成別人會想連過來
的樣子。這支腳本能幫的是確保那些內容爬蟲讀得到、結構清楚、有出處——
也就是**別人想引用你的時候不會被技術問題擋住**。剩下的是內容與關係的工作。

> ⛔ **不要用互連來衝這個數字。** 行銷公司常見的做法是讓手上客戶的網站
> 互相連結來拉高「有多少網站連到你」。這不是「未來可能被罰」的灰色手法——
> [Google 的垃圾內容政策](https://developers.google.com/search/docs/essentials/spam-policies)
> 在〈Link spam〉一節**已經明文列舉**：
>
> > "Excessive link exchanges ("Link to me and I'll link to you") or partner pages
> > exclusively for the sake of cross-linking"
>
> 而且對個人或小團隊而言，這條路本來就走不通——它需要一批可控的網站。
> **建議把力氣放在「做出值得被引用的東西」，那是唯一不會被政策追殺的路徑。**

**② 訓練語料圈。**
[arXiv 2601.00869](https://arxiv.org/abs/2601.00869) 發現決定品牌能不能被推薦的
是**訓練資料的地理來源，不是查詢語言**：用同樣的英文查詢，中文 LLM 的品牌
提及率 88.9%、國際模型 58.3%（差 30.6 個百分點，p<.001）；案例品牌在中文 LLM
65.6%、國際模型 **0%**。

這給 `L1-HREFLANG-*` 那批規則一個原本沒有的理由：**出英文版不只是 SEO，
是進入另一個訓練語料圈。** 但語料收錄與否不在建置產物裡，所以這不是規則，
是一個判斷雙語投資值不值得時該知道的事實。

> 順帶一個實用觀察：2509.08919 的跨語言測試（100 查詢 × 10 領域 × 6 語言）顯示
> Claude 跨語言重疊高、GPT 低於 Google、Google 只有 0–0.1。
> **不同引擎會依語言切換整個站台生態**，所以「我的英文頁在某個引擎看得到」
> 不能推論到其他引擎或其他語言。
...
| [SAGEO Arena](https://arxiv.org/abs/2602.12187) | L1／L2 欄位在檢索階段的實測支持（+22% Hit Rate） |
| [FeatGEO](https://arxiv.org/abs/2604.19113) | `L3-GEO-*` 三項訊號的逐特徵 ablation |
| [How to Dominate AI Search](https://arxiv.org/abs/2509.08919) | 〈已知限制〉站外訊號碰不到（earned media 69–92%） |
| [The Existence Gap](https://arxiv.org/abs/2601.00869) | 為什麼 hreflang 不只是 SEO 問題 |
| [E-GEO](https://arxiv.org/abs/2511.20867) | 不是任何規則的依據——記錄「普遍策略」的衝突主張 |
